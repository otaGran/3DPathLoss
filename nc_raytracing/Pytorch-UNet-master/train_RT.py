...
def train_model(
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=150)  # goal: maximize Dice score
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                images, true_masks = batch['combined_input'], batch['ground_truth']
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    loss = torch.sqrt(criterion(masks_pred.squeeze(1), true_masks.float()))
...
def get_args():
    ...
    parser.add_argument('--learning-rate', '-l', metavar='LR', type=float, default=1e-1,
