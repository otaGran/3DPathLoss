...
def sionna_run(height_file, extra_height, cm_cell_size, BASE_PATH_BLENDER, BASE_PATH_SIONNA, outer_idx):
    ...
    BASE_PATH_SIONNA = BASE_PATH_SIONNA
    ...
    def cm_routine(extra_height, f_ptr_height):
        try:
            ...
            start_loc = time.time()
            cm = scene.coverage_map(max_depth=8, cm_center=[0, 0, height_at_origin + extra_height],
                                    cm_orientation=[0, 0, 0],
                                    cm_cell_size=[cm_cell_size, cm_cell_size], cm_size=[1000, 1000])
    ...
    def run_routine():
        try:
            ...
            coverage_map, scene, fName = cm_routine(extra_height=extra_height, f_ptr_height=f_ptr_H)
            print(fName)
